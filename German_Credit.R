...
# setwd("/Users/James/UIC/IDS 572 Data Mining/datasets")
...
cols <- c("CHK_ACCT","HISTORY","NEW_CAR","USED_CAR","FURNITURE","NUM_CREDITS","RADIO","EDUCATION","RETRAINING","SAV_ACCT","EMPLOYMENT","PERSONAL_STATUS","COAPPLICANT","GUARANTOR","PRESENT_RESIDENT","REAL_ESTATE","PROP_UNKN_NONE","OTHER_INSTALL","RENT","OWN_RES","JOB","TELEPHONE","FOREIGN","RESPONSE")
...
# The boxplots indicate that there is a stronger relationship between DURATION and RESPONSE, with an obvious difference in 
# median and IQR. Longer duration loans tend to have worse credit. AMOUNT also shows a difference albiet likely smaller, suggesting a relationship - good credit for larger amounts. 
...
# The density plots and ANOVA tests confirm the idea that the distribution of RESPONSE is related to all the numeric variables. 
...
chisq.test(GermanCredit$RESPONSE, GermanCredit$CHK_ACCT)$p.value
...
# From these chi-squared tests, we can see that FURNITURE, RETRAINING, CO-APPLICANT, GUARANTOR, PRESENT RESIDENT, JOB, NUMBER OF DEPENDENTS, and TELEPHONE
# do not have statistically significant relationships, with pvalues > .05, while the other factors do not. 
...
# Instead they simply attempt to find a "good" tree by searching greedily.Greedy means that, any time a split is under consideration,
...
# visually, the rpart tree we generated with the full GermanCredit data uses CHK_ACCT, DURATION, HISTORY,  AMOUNT, SAV-ACCT, USED_CAR, NUM-CRED, and GUARANTOR.
...
summary(rpModel1)
# Running summary on the decision tree model, the 'Variable importance' confirms our intuitions from the data exploration. 
...
# Confusion matrix
a <- table(pred=predict(rpModel1,GermanCredit, type="class"), true=GermanCredit$RESPONSE)
a
# Accuracy on full data
mean(predict(rpModel1,GermanCredit, type="class") == GermanCredit$RESPONSE)
...
precisiongd <- a[1,1]/(a[1,1]+a[1,2]) #precision on good credit
precisiongd
recallgd <- a[1,1]/(a[1,1]+a[2,1]) # recall on good credit
recallgd

precisionbd <- a[2,2]/(a[2,2]+a[2,1]) # precision on bad credit
precisionbd
recallbd <- a[2,2]/(a[2,2]+a[1,2]) # recall on bad credit
recallbd
...
# we need to convert these to numbers, and assign bad credit = 1, and good credit = 0.
levels(GermanCreditSc$RESPONSE)[0] <- 1
levels(GermanCreditSc$RESPONSE)[1] <- 0
...
# Solution: 
# It is difficult to know, because we need to run the model on some testing data, 
...
# Q3 (a) Develop 50/50 decision tree and evaluate accuracy of this model on testing data
...
#develop a tree on the training data using default parameters
rpModel2=rpart(RESPONSE ~ ., data=GermanCreditTrn, method="class")
...
#run the test data through model2, and create confusion table
table(pred=predict(rpModel2,GermanCreditTst, type="class"), true=GermanCreditTst$RESPONSE)
#Accuracy on testing data
mean(predict(rpModel2,GermanCreditTst, type="class") == GermanCreditTst$RESPONSE)
# Our model developed on the entire dataset, using rpart default parameters, achieved about a 70% accuracy on the testing data
...
scoreTst=predict(rpModel1,GermanCreditTst, type="prob")[,'0']  
#same as predProbTst
...
#let's try altering the minimum number of cases for a split.
...
# Q3(d)
#If we do a 70/30 training/test split of our data
